...
export const TeamBlockClient: React.FC<Props> = ({ departments, title, introText }) => {
  ...
  return (
    <div className="bg-background-even">
      <div className="w-full max-w-[1440px]  mx-auto px-6 md:px-12 overflow-hidden py-20">
        <div className="flex flex-col mb-[48px]">
          <h1 className="text-center" ref={titleRef}>
            {title}
          </h1>
          <div className="flex items-center w-full max-w-[600px] mx-auto justify-center">
            <p
              ref={introTextRef}
              className="font-brand font-normal text-center text-muted-foreground"
            >
              {introText}
            </p>
          </div>
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5">
                      <p className="font-brand text-[30px] font-bold leading-[110%] tracking-[-1%] text-primary-foreground capitalize">
                        {dept.name}
                      </p>
                    </div>
                  </div>
                  ...
                  <div
                    id={`dept-${dept.id}`}
                    className={`absolute inset-0 bg-card flex flex-col transition-opacity duration-300 delay-75 ${isActive ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
                  >
                      ...
                      {currentMembers.length === 1 ? (
                        <div className="h-full bg-card">
                          {currentMembers.map((member) => (
                            <div
                              key={member.id}
                              className="relative w-full h-full group overflow-hidden bg-muted"
                            >
                        ...
                        <div
                          className="absolute inset-0 grid gap-px transition-all duration-300 bg-border/20"
                          style={{
                        ...
                        >
                          {currentMembers.map((member) => (
                            <div key={member.id} className="relative group overflow-hidden bg-card">
                              {member.image && typeof member.image !== 'string' && (
                                <Media
                                  resource={member.image}
                                  fill
                    ...
                    {totalPages > 1 && (
                      <div className="h-9 shrink-0 flex items-center justify-between px-3 bg-card border-t border-border">
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                        ...
                        <div className="flex gap-1.5 items-center">
                          {Array.from({ length: totalPages }).map((_, p) => (
                            <div
                              key={p}
                              className={`h-1 transition-all duration-300 ${p === currPage ? 'w-4 bg-primary' : 'w-1.5 bg-muted-foreground/30'}`}
                            />
                          ))}
                        </div>
                        <button
                ...
                >
                  <div className="overflow-hidden">
                    <p className="font-brand text-[24px] font-semibold leading-[140%] tracking-[0%] text-foreground capitalize">
                      {dept.name}
                    </p>
                    {dept.description && (
                      <p className="font-brand text-[16px] font-normal leading-[160%] tracking-[1%] text-muted-foreground">
                        {dept.description}
                      </p>
                    )}
                  </div>
