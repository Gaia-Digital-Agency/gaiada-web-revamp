...
export const HomepageSections: React.FC<HomepageSectionsProps> = async ({ hero, layout }) => {
  ...
  // Section 0: Hero
  sections.push(
    <div key="hero" className="w-full lg:h-full overflow-hidden bg-transparent">
      <RenderHero {...hero} />
    </div>,
  )
  ...
  if (layout && Array.isArray(layout)) {
    for (let i = 0; i < layout.length; i++) {
      ...
      if (Block) {
        sections.push(
          <div key={`block-${i}`} className="w-full lg:h-full overflow-hidden bg-transparent">
            <Block {...block} disableInnerContainer index={i + 1} />
          </div>,
        )
      }
    }
